...
class LLMHandler:
    ...
    def change_model(self, new_model_id: str, during_llm: bool = False):
        if new_model_id not in self.model_configs:
            raise ValueError(f"Unknown model: {new_model_id}")

        if self.current_model_id == new_model_id:
            return

        if not self.is_model_usable:
            raise RuntimeError("Cannot change model because handler model state is not usable")

        if self.is_available == False and during_llm == False:
            logging.error(f"[LLMHandler] change_model is called when model is busy!")
            raise RuntimeError("Cannot change model while handler is busy")

        previous_available = self.is_available
        ...
        try:
            if self.is_npu:
                ...
                try:
                    previous_model.dispose() # type: ignore
                    new_tokenizer = self.tokenizers[new_model_id]
                    new_model = self.models[new_model_id]
                    new_model.launch()
                except Exception:
                    logging.error(f"[LLMHandler] Failed to launch model: {new_model_id}", exc_info=True)
                    self.tokenizer = previous_tokenizer
                    self.model = previous_model
                    self.current_model_id = previous_model_id

                    try:
                        previous_model.launch() # type: ignore
                    except Exception:
                        logging.error("[LLMHandler] Failed to restore previous model", exc_info=True)
                        restore_failed = True
                        self.is_model_usable = False
                    raise
            ...
            if restore_failed:
                ...
                self.is_available = previous_available and during_llm == False
    ...
    def generate_response(
        ...
        if not self.is_model_usable:
            logging.error(f"[LLMHandler] generate_response is called when model state is not usable!")
            return True, ""
        ...
        try:
            ...
            self.is_available = self.is_model_usable
