...
export default function Home() {
  ...
  function onConnect() {
    ...
    setClient((client) => {
      const newClient = JSON.parse(JSON.stringify(defaultLLMClient));
  ...
  function onCurrentModel(model: string) {
    ...
    setClient((client) => {
      ...
      if (client.state == LLMState.CHANGING_MODEL)
        newClient.state = LLMState.IDLE;
      else if (![LLMState.ASKING, LLMState.ANSWERING, LLMState.ABORTING].includes(client.state))
        newClient.state = LLMState.IDLE;
  ...
  function onStart() {
    ...
    setClient((client) => {
      const newClient = JSON.parse(JSON.stringify(client));
      if (![LLMState.APPLYING_LANGUAGE, LLMState.CHANGING_MODEL].includes(client.state))
  ...
  function onToken(token: string) {
    ...
    setClient((client) => {
      ...
      const old = newClient.recentAnswer;
  ...
  function changeModel(model: string) {
    if (
      ...
      ![LLMState.CHANGING_MODEL, LLMState.APPLYING_LANGUAGE].includes(client.state)
    ) {
